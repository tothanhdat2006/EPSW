...
export const POST: RequestHandler = async ({ params, request, platform, locals }) => {
	...
	).bind(department, locals.user.id, note, now, doc['id']).run();
	...
	if (citizenEmail) {
		...
		try {
			await emailService.notifyDocumentAssigned(citizenEmail, 'Quý công dân', trackingCode, deptLabel);
		} catch (e) {
	...
	if (ctx?.waitUntil) {
		ctx.waitUntil(generateAISummaryJob(doc['id'] as string, db, platform));
	} else if ((platform as any)?.waitUntil) {
		(platform as any).waitUntil(generateAISummaryJob(doc['id'] as string, db, platform));
	} else {
		generateAISummaryJob(doc['id'] as string, db, platform).catch(console.error);
	}
...
async function generateAISummaryJob(docId: string, db: D1Database, platform: App.Platform | undefined) {
	...
	let rawFileUrlStr = '';
	try {
		const row = await db.prepare('SELECT extracted_data, raw_file_url FROM document WHERE id = ?').bind(docId).first<{ extracted_data: string, raw_file_url: string }>();
...
QUAN TRỌNG: TUYỆT ĐỐI CHỈ SỬ DỤNG thông tin có trong dữ liệu được cung cấp ở trên. KHÔNG ĐƯỢC tự bịa đặt, suy diễn, hoặc thêm bất kỳ thông tin nào bên ngoài. Không giải thích, không output thêm gì ngoài nội dung Tờ trình.`;
	...
	// Update DB
	try {
		const row = await db.prepare('SELECT extracted_data FROM document WHERE id = ?').bind(docId).first<{ extracted_data: string }>();
		let parsed: Record<string, unknown> = {};
		if (row?.extracted_data) {
			try { parsed = JSON.parse(row.extracted_data); } catch (e) {}
		}
		parsed['ai_summary'] = summary;
		parsed['ocr_extracted_text'] = extractedDataText;
		await db.prepare('UPDATE document SET extracted_data = ? WHERE id = ?').bind(JSON.stringify(parsed), docId).run();
	} catch (e) {
